...
from bso.server.main.acknowledgments import enrich_with_acknowledgments
from bso.server.main.teds import add_teds_predictions
from bso.server.main.tags import add_tags
...
def etl(args):
    ...
    if extract:
        ...
        if 'bso3' in datasources:
            # TODO final_for_bso_20251117
            #download_prefix_bso3 = args.get('bso3_file', 'final_for_bso_2025_v2')
            download_prefix_bso3 = args.get('bso3_file', 'final_for_bso_20251117')
            extract_container('bso3_publications_dump', bso_local_dict, skip_download=False, download_prefix=download_prefix_bso3, one_by_one=True, filter_fr=True, min_year=None, collection_name=collection_name, locals_data=locals_data) #always fr
...
def update_local_affiliations(publications, bso_local_dict, hal_struct_id_dict, hal_coll_code_dict, nnt_etab_dict):
    ...
    for p in publications:
        ...
        if isinstance(p.get('hal_collection_code'), list):
            p['bso_local_affiliations'] += p.get('hal_collection_code')
            for coll in p.get('hal_collection_code', []):
                if coll.lower() in hal_coll_code_dict:
                    p['bso_local_affiliations'].append(hal_coll_code_dict[coll.lower()])
        if isinstance(p.get('hal_struct_id'), list):
            p['bso_local_affiliations'] += p.get('hal_struct_id')
            for hal_struct_id in p.get('hal_struct_id', []):
                ...
                if hal_struct_id_s in hal_struct_id_dict:
                    p['bso_local_affiliations'].append(hal_struct_id_dict[hal_struct_id_s])
        nnt_id = p.get('nnt_id')
        if isinstance(nnt_id, str) and get_code_etab_nnt(nnt_id, nnt_etab_dict) in nnt_etab_dict:
            p['bso_local_affiliations'].append(nnt_etab_dict[get_code_etab_nnt(nnt_id, nnt_etab_dict)])
        x = [str(k).lower() for k in p['bso_local_affiliations']]
        x = list(set(x))
        x.sort()
