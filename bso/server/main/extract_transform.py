...
def extract_all(index_name, observations, reset_file, extract, transform, load, affiliation_matching, entity_fishing, skip_download, chunksize, datasources, hal_date, theses_date, start_chunk, reload_index_only, save_to_mongo):
    ...
    if extract:
        ...
        elif 'bso' in index_name:
            split_file(input_dir = '/upw_data', file_to_split = output_file, nb_lines = 500000, split_prefix = scanr_split_prefix, output_dir='/upw_data/bso-split', split_suffix = '_extract.jsonl')
    ...
    if transform:
        ...
        for c in df_chunks:
            ...
            elasticimport = f"elasticdump --input={enriched_output_file} --output={es_host}{index_name} --type=data --limit 500 --noRefresh " + "--transform='doc._source=Object.assign({},doc)'"
        ...
        for c in df_chunks:
            ...
            if save_to_mongo:
                ...
                for p in publications_scanr:
                    new_elt = {'id': p['id']}
                    for f in ['authors', 'domains', 'keywords', 'year', 'affiliations', 'title', 'source', 'grants']:
                        if p.get(f):
                            new_elt[f] = p[f]
...
def save_to_mongo_publi_indexes():
    ...
    mycol.create_index('grants.grantid')
    myclient.close()
