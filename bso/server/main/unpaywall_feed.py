...
small_url  = f"http://api.unpaywall.org/daily-feed/changefile/changed_dois_with_versions_2021-01-08T080001.jsonl.gz?api_key={UPW_API_KEY}"
medium_url = f"http://api.unpaywall.org/feed/changefile/changed_dois_with_versions_2020-12-15T080001_to_2020-12-24T080001.jsonl.gz?api_key={UPW_API_KEY}"
url_snapshot  = f"http://api.unpaywall.org/feed/snapshot?api_key={UPW_API_KEY}"
url = url_snapshot

def snapshot_to_mongo(f, global_metadata = False):
    ...
    jq_oa = f"zcat {f} | "
    if global_metadata:
        jq_oa += "jq -r -c '{doi, genre, is_paratext, journal_issns, journal_issn_l, journal_name, published_date, publisher, title, year, z_authors}'"
    else:
    ...
    start = datetime.datetime.now()
    mongoimport = f"mongoimport --numInsertionWorkers 2 --uri mongodb://mongo:27017/unpaywall --file {output_json} --collection {collection_name}"
    ...
    resp = mycol.create_index("doi")
    resp = mycol.create_index("year")
    resp = mycol.create_index("is_oa")
    resp = mycol.create_index("publisher")
...
def download_snapshot(asof = None, upload_to_object_storage = True ):
    try:
...
def download_daily(dt):
    daily_files = requests.get(f"https://api.unpaywall.org/feed/changefiles?api_key={UPW_API_KEY}&interval=day").json()['list']
